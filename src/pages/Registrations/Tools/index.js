...
const RegisterTools = () => {
  ...
  function handleCallApi() {
    api.get('/tools/index').then(response => setIndustries(response.data))
  }
  ...
  async function handleRegisterTool(data, { reset }) {
    try {
      ...
      api
        ...
        .then(res => {
          if (res.status === 400) alert('Erro! Ferramenta já cadastrada!')
        })
        .catch(err => {
          if (err) {
  ...
  return (
    <Layout page="Ferramentas do sistema">
      <S.Container>
        <S.HeaderWrapper>
          <Button02
            label="Cadastrar"
